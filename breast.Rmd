...
```{r}
# plotCounts(dds_HER2_luminalA,
#            gene=which.min(result_HER2_luminalA$padj),
#            intgroup="condition")
```  

비록 한번에 많은 (또는 모든) 유전자를 그려내는 것이 도움이 되긴 하지만, 특정 유전자에 대한 count가 어떻게 변화하는지 보는 것도 좋다.  

가장 유의한 차이를 보이는 유전자에 대한 그림이다.  
